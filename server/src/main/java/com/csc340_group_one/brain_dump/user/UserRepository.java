...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT u.* FROM users u JOIN articles a ON a.author_id = u.user_id WHERE a.article_id = ?1", nativeQuery = true)
    public Optional<User> getUserByArticleCreation(Long articleId);

    @Query(value = "SELECT u.* FROM users u JOIN collections c ON c.author_id = u.user_id WHERE c.collection_id = ?1", nativeQuery = true)
